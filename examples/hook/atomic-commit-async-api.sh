...
#
# Same logic as atomic-commit-async.sh but generates commit messages via a direct
# OpenAI-compatible API call instead of spawning `opencode run`. This eliminates:
#   - Go binary startup / config-load overhead
#   - Agent loop and TUI rendering
#   - ANSI stripping / output parsing
#
# Works with any OpenAI-compatible API (OpenAI, Azure, Ollama, LM Studio,
# vLLM, llama.cpp, LiteLLM, etc.) — just set the base URL, model, and key.
#
# Requires:
...
#
# Usage:
#   Hook mode: receives OpenCode file.changed JSON on stdin.
#     hooks:
#       - event: file.changed
#         async: true
#         scope: all
...
generate_commit_msg() {
  ...
  # Direct API call — no process spawn overhead
  if [[ -n "${OPENAI_API_KEY:-}" ]]; then
    debug "calling API: model=$OPENAI_MODEL timeout=${OPENAI_API_TIMEOUT}s"

    # Build JSON payload via python to handle escaping correctly
    local json_payload
...
' "$SYSTEM_PROMPT" "$user_msg" "$OPENAI_MODEL")

    local api_response
    api_response=$(curl -s --max-time "$OPENAI_API_TIMEOUT" \
    ...
    if [[ -n "$api_response" ]]; then
      ...
      commit_msg=$(printf '%s' "$api_response" | python3 -c '
import json, sys
try:
...
' 2>/dev/null || true)
      ...
      if [[ -n "$commit_msg" ]] && validate_commit_msg "$commit_msg"; then
        debug "API returned valid commit message"
        sanitize_commit_message "$commit_msg"
        return 0
      else
        debug "API response failed validation or was empty: ${commit_msg:-<empty>}"
      fi
    else
      debug "API call returned empty response"
    fi
  else
    debug "OPENAI_API_KEY not set — skipping API call"
  fi
...
commit_staged() {
  ...
  if git commit -m "$commit_msg" 2>&1; then
    ...
    log "atomic-commit-async-api: commit failed for $label" >&2
  fi
}
...
run_hook_mode() {
  debug "=== hook invoked (api variant) ==="

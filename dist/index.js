...
import { extname, isAbsolute, matchesGlob, relative } from "path";
...
var DEFAULT_BASH_TIMEOUT = 60000;

// src/core/bash-executor.ts
var TIMEOUT_EXIT_CODE = 1;
var BLOCKING_EXIT_CODE = 2;
var KILL_GRACE_PERIOD_MS = 250;
var BASH_EXECUTABLE = process.env.OPENCODE_HOOKS_BASH_EXECUTABLE || "bash";
var MAX_LOG_FIELD_LENGTH = 400;
var REDACTED = "[REDACTED]";
async function executeBashHook(request) {
...
function mapBashProcessResultToHookResult(result, context) {
  if (result.timedOut) {
...
function isBlockingToolBeforeEvent(event) {
  return event.startsWith("tool.before.");
}
async function executeBashProcess(request) {
...
function logBashOutcome(result, request) {
...
function redactSensitiveContent(value) {
  return value.replace(/\b(authorization\s*:\s*bearer\s+)([^\s]+)/gi, `$1${REDACTED}`).replace(/((?:\\?["'])?(?:api[-_ ]?key|token|secret|password|passwd|pwd)(?:\\?["'])?[^\S\r\n]*[:=][^\S\r\n]*)(\\?["'])(.*?)(\2)/gi, (_match, prefix, openingQuote, _secretValue, closingQuote) => `${prefix}${openingQuote}${REDACTED}${closingQuote}`).replace(/(["']?(?:api[-_ ]?key|token|secret|password|passwd|pwd)["']?[^\S\r\n]*[:=][^\S\r\n]*)([^\s,"'}\]`]+)/gi, `$1${REDACTED}`);
}
function resolveExecutionContext(projectDir) {
  try {
...
// src/core/types.ts
var SESSION_HOOK_EVENTS = ["session.idle", "session.created", "session.deleted", "file.changed", "message.updated", "message.part.updated"];
var LEGACY_HOOK_CONDITIONS = ["matchesCodeFiles"];
var PATH_HOOK_CONDITION_KEYS = ["matchesAnyPath", "matchesAllPaths"];
var HOOK_SCOPES = ["all", "main", "child"];
var HOOK_RUN_IN = ["current", "main"];
var HOOK_BEHAVIORS = ["stop"];
function isHookEvent(value) {
  return typeof value === "string" && (SESSION_HOOK_EVENTS.includes(value) || /^tool\.(before|after)\.(\*|.+)$/.test(value));
}
function isHookLegacyCondition(value) {
  return typeof value === "string" && LEGACY_HOOK_CONDITIONS.includes(value);
}
function isHookPathConditionKey(value) {
  return typeof value === "string" && PATH_HOOK_CONDITION_KEYS.includes(value);
}
function isHookScope(value) {
  return typeof value === "string" && HOOK_SCOPES.includes(value);
}
function isHookRunIn(value) {
  return typeof value === "string" && HOOK_RUN_IN.includes(value);
}
function isHookBehavior(value) {
  return typeof value === "string" && HOOK_BEHAVIORS.includes(value);
}
...
function resolveHookConfigPaths(options = {}) {
  const exists = options.exists ?? existsSync;
  const platform = options.platform ?? process.platform;
  const homeDir = options.homeDir ?? os.homedir();
  const appDataDir = options.appDataDir ?? process.env.APPDATA;
  const globalConfigDir = path2.join(homeDir, ".config", "opencode", "hook");
  const projectDir = options.projectDir;
  const preferredGlobal = path2.join(globalConfigDir, "hooks.yaml");
  const appDataGlobal = platform === "win32" && appDataDir ? path2.join(appDataDir, "opencode", "hook", "hooks.yaml") : undefined;
  return {
    global: platform === "win32" && appDataGlobal && !exists(preferredGlobal) ? appDataGlobal : preferredGlobal,
    project: projectDir ? path2.join(projectDir, ".opencode", "hook", "hooks.yaml") : undefined
  };
}
function discoverHookConfigPaths(options = {}) {
  const exists = options.exists ?? existsSync;
  const paths = resolveHookConfigPaths(options);
  return [paths.global, paths.project].filter((filePath) => Boolean(filePath && exists(filePath)));
}

// src/core/load-hooks.ts
function parseHooksFile(filePath, content) {
  const document = import_yaml.default.parseDocument(content);
...
function loadDiscoveredHooksSnapshot(options = {}) {
  ...
  return {
    ...
    signature: JSON.stringify(snapshots.map((snapshot) => ("content" in snapshot) ? [snapshot.filePath, snapshot.content] : [snapshot.filePath, `__read_error__:${snapshot.readError}`]))
  };
}
function loadDiscoveredHooksFromSnapshots(snapshots) {
  const hooks = new Map;
  const errors = [];
...
function snapshotDiscoveredHookFiles(files, readFile) {
...
function loadSnapshotHooksFile(snapshot) {
...
function mergeHookMapsInto(target, source) {
  for (const [event, configs] of source) {
    target.set(event, [...target.get(event) ?? [], ...configs]);
  }
}
function parseHookDefinition(filePath, hookDefinition, index, seenIds) {
...
function resolveOverrides(hooks, overrides) {
  const orderedHooks = flattenHookMap(hooks);
  const errors = [];
  for (const override of overrides) {
    const hookIndexById = new Map;
    ...
    if (targetIndex === undefined) {
      errors.push(createError(override.source.filePath, "override_target_not_found", `hooks[${override.source.index}].override targets unknown hook id "${override.targetId}".`, `hooks[${override.source.index}].override`));
      continue;
    }
    if (override.disable) {
...
function parseScope(filePath, scope, index) {
  if (scope === undefined) {
...
function parseRunIn(filePath, runIn, index) {
  if (runIn === undefined) {
...
function parseAsync(filePath, async_, event, actions, index) {
  if (async_ === undefined) {
    return { errors: [] };
  }
...
function parseHookAction(filePath, action, event, index) {
  if (action === undefined) {
    return { errors: [] };
  }
...
function parseConditions(filePath, conditions, event, index) {
...
function parseStructuredCondition(filePath, condition, event, hookIndex, conditionIndex) {
  ...
  if (keys.length !== 1) {
    return {
      error: createError(filePath, "invalid_conditions", `${conditionPath} must define exactly one supported condition key.`, conditionPath)
    };
  }
  const [key] = keys;
  ...
  if (!supportsPathConditions(event)) {
    return {
      error: createError(filePath, "invalid_conditions", `${conditionPath}.${key} is only supported on file.changed and session.idle hooks.`, `${conditionPath}.${key}`)
    };
  }
  ...
  return { condition: { [key]: values.values } };
}
function normalizePathConditionValues(value, path3) {
  ...
  if (!Array.isArray(value)) {
    return {
      error: {
        message: `${path3} must be a non-empty string or non-empty string array.`,
        path: path3
      }
    };
  }
  if (value.length === 0) {
    return {
      error: {
        message: `${path3} must not be an empty array.`,
        path: path3
      }
    };
  }
  ...
  if (invalidIndex >= 0) {
    return {
      error: {
        message: `${path3}[${invalidIndex}] must be a non-empty string.`,
        path: `${path3}[${invalidIndex}]`
      }
    };
  }
  return { values: [...value] };
}
function supportsPathConditions(event) {
  return event === "file.changed" || event === "session.idle";
}
function parseActions(filePath, actions, index) {
...
function parseAction(filePath, action, hookIndex, actionIndex) {
  const path3 = `hooks[${hookIndex}].actions[${actionIndex}]`;
  if (!isRecord(action)) {
    return { errors: [createError(filePath, "invalid_action", `${path3} must be an object.`, path3)] };
  }
  const keys = ["command", "tool", "bash"].filter((key) => (key in action));
  if (keys.length !== 1) {
    return { errors: [createError(filePath, "invalid_action", `${path3} must define exactly one of command, tool, or bash.`, path3)] };
  }
  if ("command" in action) {
    const command = parseCommandAction(action.command);
    return command ? { action: { command }, errors: [] } : { errors: [createError(filePath, "invalid_action", `${path3}.command must be a string or { name, args? }.`, `${path3}.command`)] };
  }
  if ("tool" in action) {
    const tool = parseToolAction(action.tool);
    return tool ? { action: { tool }, errors: [] } : { errors: [createError(filePath, "invalid_action", `${path3}.tool must be { name, args? }.`, `${path3}.tool`)] };
  }
  const bash = parseBashAction(action.bash);
  return bash ? { action: { bash }, errors: [] } : { errors: [createError(filePath, "invalid_action", `${path3}.bash must be a string or { command, timeout? }.`, `${path3}.bash`)] };
}
function parseCommandAction(value) {
  if (isNonEmptyString(value)) {
    return value;
  }
...
function parseBashAction(value) {
  if (isNonEmptyString(value)) {
    return value;
  }
...
function createError(filePath, code, message, errorPath) {
  return {
    code,
    filePath,
    message,
...
function formatHookReadError(error) {
...
function parseHookId(filePath, id, index, seenIds) {
  if (id === undefined) {
    return { errors: [] };
  }
...
function parseOverrideTarget(filePath, override, disable, index) {
  const errors = [];
...
function flattenHookMap(hooks) {
  return Array.from(hooks.values()).flat();
}
function toHookMap(hooks) {
...
class SessionStateStore {
  ...
  rememberSession(sessionID, parentID) {
  ...
  async evaluateScope(sessionID, scope, resolveParentID) {
    if (scope === "all") {
      return true;
    }
  ...
  async getRootSessionID(sessionID, resolveParentID) {
  ...
  setPendingToolCall(callID, sessionID, toolArgs) {
    this.pendingToolCalls.set(callID, { sessionID, toolArgs });
  }
  consumePendingToolCall(callID) {
  ...
  addFileChanges(sessionID, changes) {
    const record = this.getOrCreateSession(sessionID);
  ...
  addUserMessage(sessionID, messageID) {
    this.getOrCreateSession(sessionID).userMessageIDs.add(messageID);
  }
  isUserMessage(sessionID, messageID) {
  ...
  async resolveRootSessionID(sessionID, resolveParentID, visited) {
    const record = this.getOrCreateSession(sessionID);
...
function serializeFileChange(change) {
  if (change.operation === "rename") {
...
// src/core/tool-paths.ts
var DIRECT_MUTATION_TOOL_NAMES = ["write", "edit", "multiedit"];
var PATCH_MUTATION_TOOL_NAMES = ["patch", "apply_patch"];
var BASH_TOOL_NAME = "bash";
var MUTATION_TOOL_NAMES = new Set([...DIRECT_MUTATION_TOOL_NAMES, ...PATCH_MUTATION_TOOL_NAMES, BASH_TOOL_NAME]);
function normalizeMutationToolName(toolName) {
  if (DIRECT_MUTATION_TOOL_NAMES.includes(toolName)) {
    return toolName;
  }
  if (PATCH_MUTATION_TOOL_NAMES.includes(toolName)) {
    return "apply_patch";
  }
...
function getMutationToolHookNames(toolName) {
...
function getToolFileChanges(toolName, args) {
  const normalized = normalizeMutationToolName(toolName);
  if (!normalized) {
    return [];
  }
...
function getChangedPaths2(changes) {
  const paths = new Set;
  for (const change of changes) {
    if (change.operation === "rename") {
      if (change.fromPath) {
...
function parseBashChanges(command) {
  const changes = [];
...
function splitBashCommands(command) {
...
function extractPathArgs(tokens, startIndex) {
  const paths = [];
...
function createHooksRuntime(input, options = {}) {
  let loaded = options.hooks ? { hooks: options.hooks, errors: [], signature: "manual" } : loadDiscoveredHooksSnapshot({ projectDir: input.directory });
  ...
  const dispatchStates = new Map;
  const asyncQueues = new Map;
  const actionRecursionGuards = new AsyncLocalStorage;
  ...
  return {
    "tool.execute.before": async (eventInput, eventOutput) => {
      ...
      const result = await dispatchToolHooks(activeHooks, state, input, runBashHook, dispatchStates, actionRecursionGuards, asyncQueues, "before", eventInput.tool, sessionID, {
    ...
    "tool.execute.after": async (eventInput, _eventOutput) => {
      const activeHooks = refreshHooks();
      const sessionID = eventInput.sessionID;
      if (!sessionID) {
        return;
      ...
      const files = changes.length > 0 ? getChangedPaths2(changes) : undefined;
      ...
      await dispatchToolHooks(activeHooks, state, input, runBashHook, dispatchStates, actionRecursionGuards, asyncQueues, "after", eventInput.tool, sessionID, {
        files,
        changes,
        toolName: eventInput.tool,
        toolArgs
    ...
    event: async ({ event }) => {
      ...
      if (event.type === "session.created") {
        ...
        const sessionID = pickString2(info?.id);
        if (!sessionID) {
          return;
        }
        state.rememberSession(sessionID, pickString2(info?.parentID) ?? null);
      ...
      if (event.type === "session.deleted") {
        ...
        state.rememberSession(sessionID, pickString2(info?.parentID) ?? undefined);
      ...
      if (event.type === "session.idle") {
        const sessionID = pickString2(properties.sessionID);
        if (!sessionID) {
          return;
        }
      ...
      if (event.type === "message.updated") {
        const sessionID = pickString2(properties.sessionID);
        const messageID = pickString2(properties.messageID);
        const role = pickString2(properties.role);
      ...
      if (event.type === "message.part.updated") {
        const sessionID = pickString2(properties.sessionID);
        const messageID = pickString2(properties.messageID);
        const text = pickString2(properties.text) ?? "";
...
async function dispatchToolHooks(hooks, state, input, runBashHook, dispatchStates, actionRecursionGuards, asyncQueues, phase, toolName, sessionID, context) {
  const wildcardResult = await dispatchHooks(hooks, state, input, runBashHook, `tool.${phase}.*`, sessionID, context, { canBlock: phase === "before" }, dispatchStates, actionRecursionGuards, asyncQueues);
  ...
  for (const resolvedToolName of getMutationToolHookNames(toolName).length > 0 ? getMutationToolHookNames(toolName) : [toolName]) {
    const result = await dispatchHooks(hooks, state, input, runBashHook, `tool.${phase}.${resolvedToolName}`, sessionID, context, { canBlock: phase === "before" }, dispatchStates, actionRecursionGuards, asyncQueues);
    if (result.blocked) {
      return result;
    }
  }
...
async function dispatchHooks(hooks, state, input, runBashHook, event, sessionID, context = {}, options = {}, dispatchStates, actionRecursionGuards, asyncQueues) {
  ...
  let currentResult = { blocked: false };
  let currentError;
  try {
  ...
  async function executeDispatchRequest(request) {
...
async function executeHook(hook, state, input, runBashHook, sessionID, context, options, actionRecursionGuards, asyncQueues) {
  try {
  ...
  if (hook.async) {
    ...
    const next = previous.then(async () => {
      for (const action of hook.actions) {
        await executeAction(action, hook.runIn, input, state, runBashHook, hook.event, sessionID, context, hook.source.filePath, actionRecursionGuards);
      }
    }).catch((error) => {
      logHookFailure(hook.event, hook.source.filePath, error);
    }).finally(() => {
  ...
  for (const action of hook.actions) {
    const result = await executeAction(action, hook.runIn, input, state, runBashHook, hook.event, sessionID, context, hook.source.filePath, actionRecursionGuards);
...
async function shouldRunHook(hook, state, input, sessionID, context) {
...
function getFinalChangedPaths(projectDir, context) {
...
function normalizeConditionPath(projectDir, filePath) {
...
function normalizeGlobCandidate(filePath) {
  return filePath.replaceAll("\\", "/").replace(/^\.\//, "");
}
async function executeAction(action, runIn, input, state, runBashHook, event, sessionID, context, sourceFilePath, actionRecursionGuards) {
  ...
  if ("command" in action) {
    try {
      ...
      const config2 = typeof action.command === "string" ? { name: action.command, args: "" } : action.command;
      await withActionRecursionGuard(actionRecursionGuards, actionKey, () => input.client.session.command({
        path: { id: targetSessionID },
        body: {
          command: config2.name,
          arguments: config2.args ?? ""
        },
  ...
  if ("tool" in action) {
    try {
      ...
      await withActionRecursionGuard(actionRecursionGuards, actionKey, () => input.client.session.prompt({
        path: { id: targetSessionID },
        body: {
          parts: [
            {
...
async function resolveActionSessionID(state, input, sessionID, runIn) {
  const targetSessionID = runIn === "main" ? await state.getRootSessionID(sessionID, (currentSessionID) => resolveParentSessionID(input, currentSessionID)) : sessionID;
  return state.isDeleted(targetSessionID) ? undefined : targetSessionID;
}
async function abortSession(input, sessionID) {
  try {
...
function formatHookReloadErrors(errors) {
  const details = errors.map((error) => `${error.filePath}${error.path ? `#${error.path}` : ""}: ${error.message}`);
...
function resolveToolArgs(eventArgs, pendingArgs) {
  if (eventArgs && Object.keys(eventArgs).length > 0) {
    return eventArgs;
  }
  return pendingArgs ?? eventArgs ?? {};
}
function logHookFailure(event, filePath, error) {
  const message = error instanceof Error ? error.message : String(error);
  console.error(`[opencode-yaml-hooks] ${event} hook from ${filePath} failed: ${message}`);
}
async function withActionRecursionGuard(actionRecursionGuards, actionKey, execute) {
  ...
  const rootKeys = new Set([actionKey]);
...
async function createOpencodeHooksPlugin(_input) {
  return createHooksRuntime(_input);
}

// src/index.ts
var OpencodeHooksPlugin = async (input) => {
  return createOpencodeHooksPlugin(input);
};
var src_default = OpencodeHooksPlugin;
export {
